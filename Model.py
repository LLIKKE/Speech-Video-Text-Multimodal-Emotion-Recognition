...
class Model(nn.Module):
    def __init__(self, opt):
        ...
        # MLPsEncoder
        self.mlp_encoder = MLPEncoder(activate=opt.activate, d_in=[opt.time_len, opt.models, d_common], d_hiddens=opt.d_hiddens, d_outs=opt.d_outs, dropouts=opt.dropout_mlp, bias=opt.bias, ln_first=opt.ln_first, res_project=opt.res_project)

        # Define the Classifier
        classify_dim = get_output_dim(self.features_compose_t, self.features_compose_k, opt.d_outs[-1][2], opt.d_outs[-1][0], 3)
        if classify_dim <= 128:
