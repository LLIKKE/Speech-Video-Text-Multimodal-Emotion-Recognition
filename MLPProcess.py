...
from BC_Atten import BidirectionalCrossAttention
...
class MLPsBlock(nn.Module):    
    def __init__(self, activate, d_ins, d_hiddens, d_outs, dropouts, bias, ln_first=False, res_project=False):
        ...
        #self.mlp_k = MLP(activate, d_ins[1], d_hiddens[1], 1, bias)
        ...
        if ln_first:
            ...
            #self.ln_k = nn.LayerNorm(1, eps=1e-6)
        ...
        if not res_project:
            ...
            #self.res_projection_k = nn.Linear(d_ins[1], 1, bias=False)
            self.res_projection_d = nn.Linear(d_ins[2], d_outs[2], bias=False)
        self.atten = BidirectionalCrossAttention(
        dim = d_outs[2],
        heads = 8,
        dim_head = d_outs[2]//8,
        #context_dim = 386
    ...
    def forward_ln_first(self, x, mask):
        ...
        residual_a = torch.sum(x,dim=2)
        x = self.atten(x[:,:,0,:],x[:,:,1,:])
        x=x+residual_a
        return x
...
class MLPEncoder(nn.Module):
    def __init__(self, activate, d_in, d_hiddens, d_outs, dropouts, bias, ln_first=False,
                 res_project=[False, False, True]):
        super(MLPEncoder, self).__init__()
        assert len(d_hiddens) == len(d_outs) == len(res_project)
        self.layers_stack_stage1 = nn.ModuleList([
            MLPsBlock(activate=activate, d_ins=d_in if i == 0 else d_outs[i - 1], d_hiddens=d_hiddens[i],
                      d_outs=d_outs[i], dropouts=dropouts, bias=bias, ln_first=ln_first, res_project=res_project[i])
            for i in range(len(d_hiddens))])

        self.layers_stack_stage2 = nn.ModuleList([
            MLPsBlock(activate=activate, d_ins=d_in if i == 0 else d_outs[i - 1], d_hiddens=d_hiddens[i],
                      d_outs=d_outs[i], dropouts=dropouts, bias=bias, ln_first=ln_first, res_project=res_project[i])
            for i in range(len(d_hiddens))])

        self.layers_stack_stage3 = nn.ModuleList([
            MLPsBlock(activate=activate, d_ins=d_in if i == 0 else d_outs[i - 1], d_hiddens=d_hiddens[i],
                      d_outs=d_outs[i], dropouts=dropouts, bias=bias, ln_first=ln_first, res_project=res_project[i])
            for i in range(len(d_hiddens))])
    def combination(self,x):
        ...
        x1 = torch.cat([x[:,:,0:1,:],x[:,:,1:2,:]],dim=2)
        x2 = torch.cat([x[:,:,0:1,:],x[:,:,2:3,:]],dim=2)
        x3 = torch.cat([x[:,:,1:2,:],x[:,:,2:3,:]],dim=2)
        return [x1,x2,x3]

    def forward(self, x, mask=None):
        for enc_layer1,enc_layer2,enc_layer3 in zip(self.layers_stack_stage1,self.layers_stack_stage2,self.layers_stack_stage3):
            #print(x.shape)
            hiddens = self.combination(x)   # [b,length,dim] * 3
            #print(enc_layer1(hiddens[0]).shape,enc_layer2(hiddens[1]).shape,enc_layer3(hiddens[2]).shape)
            x = torch.stack([enc_layer1(hiddens[0]),enc_layer2(hiddens[1]),enc_layer3(hiddens[2])],dim=2)
        return x
...
if __name__ == '__main__':
    ...
    encoder = to_gpu(MLPEncoder(activate='gelu', d_in=[100, 2, 128], \
                                d_hiddens=[[50, 2, 128],[10,2,32]],\
                                d_outs=[[50,2,128],[10,2,32]], dropouts=[0.3,0.5,0.6], bias=False, ln_first=True, res_project=[True, True]))
